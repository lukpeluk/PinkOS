...
static uint32_t uintToBase(uint64_t value, unsigned char * buffer, uint32_t base);
...
void ncPrintInPlace(const unsigned char * string)
{
	int i;
...
void ncNewline()
{
	...
	*/
	...
	currentVideo += (width * 2) - (currentVideo - video) % (width * 2);
}
